...
def train_data_process():
    train_data=FashionMNIST(root='D:/Code/pytorch/LeNet/data',train=True,transform=transforms.Compose([transforms.Resize(227),transforms.ToTensor()]),download=True)
    train_data,val_data=Data.random_split(train_data,[round(0.8*len(train_data)),round(0.2*len(train_data))])
    train_dataloader=Data.DataLoader(dataset=train_data,batch_size=256,shuffle=True,num_workers=0)
    val_dataloader=Data.DataLoader(dataset=val_data,batch_size=256,shuffle=True,num_workers=0)
